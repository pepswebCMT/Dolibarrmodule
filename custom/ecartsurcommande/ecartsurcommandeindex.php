...
$max = 50; // Nombre maximum d'écarts à afficher
$now = dol_now();
...
if (isModEnabled('supplier_order') && isModEnabled('supplier_invoice')) {
	...
	$sql .= " INNER JOIN " . MAIN_DB_PREFIX . "element_element as ee ON (ee.fk_source = cf.rowid AND ee.sourcetype = 'order_supplier' AND ee.targettype = 'invoice_supplier')";
	$sql .= " INNER JOIN " . MAIN_DB_PREFIX . "facture_fourn as ff ON ee.fk_target = ff.rowid";
	$sql .= " LEFT JOIN " . MAIN_DB_PREFIX . "societe as s ON cf.fk_soc = s.rowid";
	$sql .= " WHERE cf.entity IN (" . getEntity('supplier_order') . ")";
	$sql .= " AND cf.fk_statut >= 3"; // Commandes validées/reçues
	$sql .= " AND ff.rowid IS NOT NULL";

	if ($socid > 0) {
		$sql .= " AND cf.fk_soc = " . ((int) $socid);
	}

	$sql .= " ORDER BY cf.date_commande DESC, cf.ref DESC, ff.datef ASC";
	$sql .= $db->plimit($max * 10, 0); // On prend plus large car on va regrouper
	...
	if ($resql) {
		...
		while ($i < $num) {
			...
			// Ajout de la facture au tableau des factures de cette commande
		...
		// Limitation au nombre max après regroupement
		$commandes_data = array_slice($commandes_data, 0, $max, true);
		...
		if (count($commandes_data) > 0) {
			...
			if ($nb_ecarts > 0) {
				...
				print '<td colspan="6">Écart moyen</td>';
			...
			print '<tr class="oddeven">';
			print '<td colspan="8" class="opacitymedium center">' . $langs->trans("NoSupplierOrdersFound") . '</td>';
			print '</tr>';
		}
